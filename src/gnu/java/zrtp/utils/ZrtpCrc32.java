 ...
 * inside the ZRTP Java implementation.
 */
...
public class ZrtpCrc32 {
    ...
    private static int CRC32C(int crc32, byte d)
    {
        return (crc32>>>8)^crc_c[(crc32^d)&0xFF];
    }
    ...
    public static boolean zrtpCheckCksum(byte[] buffer, int off, int len, int crc32)
    {
        int chksum = zrtpGenerateCksum(buffer, off, len);
        chksum = zrtpEndCksum(chksum);
        // fprintf(stderr, "Received crc %x, computed crc: %x\n", crc32, chksum);
        return (crc32 == chksum);
    }

    public static int zrtpGenerateCksum(byte[] buffer, int off, int len)
    {
        int crc32 = ~0;

        /* Calculate the CRC. */
        for (int i = 0; i < len ; i++)
            crc32 = CRC32C(crc32, buffer[off+i]);
    ...
    public static int zrtpEndCksum(int crc32)
    {
        ...
        // TODO: check if we can eliminate this byteswap for java
        // the negated crc is already in network order, thus we
        // swap into host (little endian), then again in network order
        ...
        byte0 = (byte)result;
        byte1 = (byte)(result>>8);
        byte2 = (byte)(result>>16);
        byte3 = (byte)(result>>24);
    ...
    public static void main(String[] args)
    {
        ...
        int crc = zrtpGenerateCksum(rawData, 0, rawData.length);
        System.out.println("Expected result: 0x664f75eb");
        System.out.println("Result:          0x"+ Integer.toHexString(crc));
        crc = zrtpEndCksum(crc);
        System.out.println("Expected result: 0x148ab099");
        System.out.println("End-result:      0x"+ Integer.toHexString(crc));
