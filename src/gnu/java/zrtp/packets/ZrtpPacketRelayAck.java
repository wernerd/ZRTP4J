...
public class ZrtpPacketRelayAck extends ZrtpPacketBase {
     ...
     */
    private static final int RELAY_ACK_LENGTH = 
    ...
    public ZrtpPacketRelayAck() {
        super(new byte[RELAY_ACK_LENGTH]);
        setZrtpId();
    ...
    public static void main(String[] args) {
        ZrtpPacketRelayAck pkt = new ZrtpPacketRelayAck();
        System.err.println("relayAck length: " + pkt.getLength());
        System.err.println("packetBuffer length in bytes: " + pkt.getHeaderBase().length);
        ZrtpUtils.hexdump("relayAck packet", pkt.getHeaderBase(), pkt.getHeaderBase().length);
    }
    *** */
}
