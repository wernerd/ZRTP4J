...
public class TransmitterMultiPBX {
            ...
            public void showMessage(ZrtpCodes.MessageSeverity sev,
                ...
                if (sev == ZrtpCodes.MessageSeverity.Info) {
                    ...
                    if (inf == ZrtpCodes.InfoCodes.InfoSecureStateOn) {
                        ...
                        /*
                         * The first  (primary) ZRTP session switch to secure mode. We are testing
                         * trusted PBX here: get the SAS type (rendering scheme) and an empty SAS Hash
                         * and send SASRelay to a non-enrolled client.
                         */
                        ZrtpConstants.SupportedSASTypes sasType = senderFirst.zrtpEngine.getSasType();
                        byte[] sasEmptyHash = new byte[32];
                        //sasEmptyHash[0] = 0x11;
                        senderFirst.zrtpEngine.sendSASRelayPacket(sasEmptyHash, sasType);
                    }
                    return;
                }
...
//                config.setMandatoryOnly();
                ...
                // IMPORTANT: MitM mode (trusted PBX) must be set _before_ intialization
    ...
    public static void main(String[] args) {
        ...
        TransmitterMultiPBX trans = new TransmitterMultiPBX();
        trans.doIt();
        try {
            Thread.sleep(2000);
        } catch (InterruptedException ie) {
